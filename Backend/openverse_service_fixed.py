...
class OpenverseService:
    ...
    async def search_photos(
        ...
        # ALWAYS enhance query with interior design keywords to get relevant results
        design_keywords = "interior design room house home architecture"
        
        # Build a design-focused query
        if query:
            # Remove generic words that dilute results
            clean_query = query.lower().replace("interior design architecture", "").strip()
            if clean_query:
                enhanced_query = f"{clean_query} {design_keywords}"
            else:
                enhanced_query = design_keywords
        else:
            enhanced_query = design_keywords
        ...
        params = {
            ...
            "license_type": "all",
            "extension": "jpg",
            # Add category filter for more relevant results
            "category": "photograph",  # Filter to photographs only
        }
        
        print(f"Openverse search: '{enhanced_query}'")
