...
async def test_prokerala_integration():
    ...
    if not client_id or not secret_key:
        print("[FAILED] Prokerala API credentials not found!")
        return False

    print(f"[SUCCESS] Credentials loaded - Client ID: {client_id[:8]}...")
    ...
    if not token:
        print("[FAILED] Failed to get access token")
        return False
    ...
    try:
        ...
        if astro_data:
            print("[SUCCESS] Successfully retrieved authentic astrological data!")
            ...
            print("[FAILED] Failed to get astrological data")
            return False
except Exception as e:
    print(f"[FAILED] Error during API test: {e}")
    return False
