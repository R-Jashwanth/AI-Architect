...
export const ModelViewerWrapper: React.FC<ModelViewerWrapperProps> = ({
  ...
  useEffect(() => {
    ...
    if (!modelViewer || !isScriptLoaded) return;

    console.log('Setting up Model Viewer listeners, isScriptLoaded:', isScriptLoaded);
    ...
    // Event handlers consolidated in useEffect for logging context


    modelViewer.addEventListener('load', () => {
      console.log(`[ModelViewer] Model loaded: ${src}`);
      setIsLoading(false);
      onLoad?.();
    });
    ...
    modelViewer.addEventListener('error', (event: any) => {
      ...
      console.error('[ModelViewer] Error:', errorMessage, event.detail);
    ...
    modelViewer.addEventListener('ar-status', (event: any) => {
      ...
      if (status === 'not-presenting') {
        // Just stopped presenting, don't necessarily say unsupported
      } else if (status === 'session-started') {
        setIsARSupported(true);
      } else if (status === 'failed') {
        console.error('[ModelViewer] AR failed to start');
      }
    });

    // Initial check for AR support after a short delay for component upgrade
    const checkSupport = () => {
      if (modelViewer.canActivateAR) {
        console.log('[ModelViewer] AR is supported (detected)');
        setIsARSupported(true);
      } else {
        console.log('[ModelViewer] AR support not detected (canActivateAR=false)');
  ...
  }, [src, isScriptLoaded, onLoad, onError]);
  ...
  return (
    <div className="model-viewer-wrapper relative w-full h-full">
      ...
      <model-viewer
        ...
        className="w-full h-full min-h-[400px] bg-gradient-to-b from-[#f0f9ff] to-[#e0f2fe]"
        loading="eager"
      >
        {/* Loading Indicator */}
        {isLoading && (
